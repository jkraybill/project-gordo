...
**Active (Session 3 snapshot):**

1. **Constitutional change governance** (Track A) -- ratify "all constitutional changes require ratification, single-tier" plus three drift-fix amendments. Draft at `ratification/002_constitutional_change_governance-DRAFT.md`. Awaiting JK review → multi-model adversarial → proto-MCAP ratification.
2. **Lineage model + viral propagation** (Track B, issues #3+#4 folded) -- Apache-style umbrella with three-signal disclosure and primitive/composite distinction. Draft at `docs/LINEAGE_MODEL.md` with Phase D.5 pre-staging on eight open sub-questions (five Gordo proposals, three JK-judgment). Gated behind Track A.
3. **Formal adoption of MCAP as umbrella ratification protocol** -- proto-MCAP used twice now; succession to full MCAP (v0.2.0 exists as downstream) anticipated as part of Track B or immediately after.
