...
**Status:** DRAFT -- PRE-RATIFICATION. Awaiting JK review, multi-model adversarial review, and proto-MCAP attestations.
...
3. Three drift-fix amendments to already-ratified sections ("Non-Negotiable Values" and "How These Values Relate") removing the hardcoded count "seven"

**Content hash:** *(to be computed at time of commit)*
...
> - Unratified portions of CONSTITUTION.md during bootstrap (these become subject to this rule upon ratification)
>
...
### Item 3 -- Drift-fix amendments to ratified sections
...
- **(a) Editorial exemption:** the meaning-preserving drift fix can be committed with explicit acknowledgment but no new ratification record
...
The three drift fixes in this ratification are themselves small -- meaning-preserving, scoped to removing a hardcoded count. Under an editorial-exemption regime, a future session might have made them silently. Under single-tier, they become part of the permanent record, and the rule is demonstrated by being immediately applied to the drift it surfaced.
...
2. Adopt MCAP v0.2.0 formally in this same ratification, then use MCAP for the governance rule + drift fixes (would expand Track A scope substantially)
...
Response: PROTO_MCAP.md is a historical descriptive record of the first proto-MCAP use, not a ratified constraint on future uses. Its "used once" language was aspirational, anticipating MCAP adoption that has not yet occurred. This ratification honestly names the reuse and acknowledges proto-MCAP's bootstrap role extends slightly further than first anticipated. A housekeeping update to PROTO_MCAP.md to reflect this will be made at ratification time.
...
Response: yes, in isolation. But the governance rule being established is foundational, and applying it to real drift demonstrates the rule works. Packaging them together is pedagogical: the new rule is not abstract, it has a concrete first use. Future observers of the ratification/ directory see how the rule behaves in practice.
...
Response: ratification requires mutual consent by definition. Disagreement means no ratification, which means the proposed changes are not adopted. The status quo prevails: the new subsection is not added, the bullet is not clarified, the drift fixes are not applied. Disagreement is not an emergency; it is the rule working as designed.

### Attack 10 -- "Is this rule recursive? Does amending it require this rule?"

Response: yes. Once this rule is ratified, amending it requires ratification under itself (until superseded by a formal MCAP amendment or equivalent). This is analogous to Article V of the US Constitution being amendable via Article V itself. Recursive self-application is the intended property.
...
I have reviewed the proposed new subsection, the clarified bullet, and the three drift-fix amendments in their current form. I participated in drafting, stress-testing, and refining these items through deliberation with JK during Session 3.

I attest that:
- I consent to the new subsection becoming part of CONSTITUTION.md Process Standards
- I consent to the three drift-fix amendments in already-ratified content
